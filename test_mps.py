...
def inference(text, ref_s, alpha = 0.3, beta = 0.7, diffusion_steps=5, embedding_scale=1, use_fp16=True):
    ...
    tokens.insert(0, 0)
    tokens = torch.LongTensor(tokens).unsqueeze(0)  #.to(gpu_device)
    ...
    with torch.no_grad():
        ...
        # encode prosody
        en = (d.transpose(-1, -2) @ pred_aln_trg.unsqueeze(0).to(gpu_device))
...
def LFinference(text, s_prev, ref_s, alpha = 0.3, beta = 0.7, t = 0.7, diffusion_steps=5, embedding_scale=1):
  ...
  with torch.no_grad():
      ...
      s_pred = sampler(noise = torch.randn((1, 256)).unsqueeze(1).to(gpu_device),
                                        embedding=bert_dur,
...
def STinference(text, ref_s, ref_text, alpha = 0.3, beta = 0.7, diffusion_steps=5, embedding_scale=1):
    ...
    ref_tokens = torch.LongTensor(ref_tokens).to(gpu_device).unsqueeze(0)
