...
def get_en(d_in, pred_aln_trg):
    en = (d_in @ pred_aln_trg.unsqueeze(0).to(gpu_device))
...
def inference(text, ref_s, alpha = 0.3, beta = 0.7, diffusion_steps=5, embedding_scale=1, use_fp16=True):
    ...
    with torch.no_grad():
        ...
        # encode prosody
        en = get_en(d.transpose(-1, -2), pred_aln_trg)
        F0_pred, N_pred = model.predictor.F0Ntrain(en, s)
        asr = get_en(t_en, pred_aln_trg)
